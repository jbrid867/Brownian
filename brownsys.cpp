...
void brownsys::diff_steps(int num) //num is the number of steps per generation
{
	float sigma=sqrt(6*h*D);
	cout<<"Sigma = "<<sigma<<endl;
	//cout<<"got eeemmm"<<endl;
	rand_wrapper(N*num, (float *)steps, sigma); 
	//cout<<"got eeem?"<<endl;
	ofstream pdf;
	pdf.open("steps.txt");
...
void brownsys::equilibrate(mt19937& gen, normal_distribution<> distro, int eqsteps)
{
 	...
 	for(int i=0;i<eqsteps;i++) // should be while !equilibrated
 	{
	 	...
	 	{counter++;
			...
			mag=pow(mag2,0.5);
			if(mag<crowders[k].getp("radius")+main.getp("radius")){clash=true;clashcount++;}	

			//clash with center
			mag=0;mag2=0;
			for(int dumb=0;dumb<3;dumb++){mag2+=pos1[i]*pos1[i];}
			mag=pow(mag2,0.5);
			if(mag<crowders[k].getp("radius")+crad){clash=true;clashcount++;}	
		...
		if(i%10==0){cout<<"the number of clashes after "<<i<<" equilibration steps is "<<clashcount<<endl;}
