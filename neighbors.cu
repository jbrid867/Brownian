...
__global__ void rand_gen(curandState *state, float * rands, int n, float sigma)
{
	...
	if(idx<n*n)
	{
		...
		while(random<0.0000001)
		{
			random = curand_normal(&state[idx]);
			random=random*sigma;
		}
		x=curand_uniform(&state[idx])-0.5;
		y=curand_uniform(&state[idx])-0.5;
		z=curand_uniform(&state[idx])-0.5;
		mag2=x*x+y*y+z*z;
		mag=sqrt(mag2);
